...
function CharacterSelect({gameState, updateGameState, currentChar}: CharacterSelectProps) {
    ...
    const changeCharacter = (newCharName: CharacterName, currentChar: Character) => {
        ...
        const newCharNoLongerInPlay = gameState.allChars.filter((char) => !char.inPlay && char.name === newCharName);

        // If the character we're switching away from will no longer be in play, we'll want to add a "not in play" dummy character
        const oldCharNoLongerInPlay = gameState.allChars.filter((char) => char.inPlay && char.name === oldCharName).length === 1;
        ...
        if (newCharNoLongerInPlay && newCharNoLongerInPlay.length > 0 && newCharNoLongerInPlay[0].id) {
            newCharsList = newCharsList.filter((char) => char.id !== newCharNoLongerInPlay[0].id);
        }
    ...
    return (
        <select defaultValue={currentChar.name} onChange={e => changeCharacter(e.target.value as CharacterName, currentChar)}>
