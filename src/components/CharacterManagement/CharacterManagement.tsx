...
function CharacterMangement({gameState, updateGameState}: CharacterManagementProps) {
    
    const toggleAlignment = (char: Character) => {
        const newChars = gameState.allChars.map((oldChar) => {
            if (oldChar.id === char.id) {
                return Object.assign(oldChar, { alignment: char.alignment === Alignment.Good ? Alignment.Evil : Alignment.Good });
            } else {
                return oldChar;
            }
        });
